NAME = libft.a

M_SRCS =	ft_atoi.c  ft_isprint.c  ft_memset.c  ft_strdup.c ft_strncmp.c \
		 	ft_bzero.c  ft_itoa.c  ft_putchar_fd.c  ft_striteri.c  ft_strnstr.c \
		 	ft_calloc.c  ft_putendl_fd.c  ft_strjoin.c   ft_strrchr.c \
		 	ft_isalnum.c  ft_memchr.c  ft_putnbr_fd.c  ft_strlcat.c  ft_strtrim.c \
		 	ft_isalpha.c ft_memcmp.c   ft_putstr_fd.c   ft_strlcpy.c  ft_substr.c \
		 	ft_isascii.c  ft_memcpy.c   ft_split.c  ft_strlen.c   ft_tolower.c \
		 	ft_isdigit.c  ft_memmove.c  ft_strchr.c  ft_strmapi.c   ft_toupper.c

B_SRCS =	ft_lstadd_back_bonus.c  ft_lstsize_bonus.c  ft_lstnew_bonus.c  ft_lstclear_bonus.c \
			ft_lstdelone_bonus.c  ft_lstlast_bonus.c ft_lstadd_front_bonus.c  ft_lstmap_bonus.c \
			ft_lstiter_bonus.c

M_OBJS = ${M_SRCS:.c=.o}
B_OBJS = ${B_SRCS:.c=.o}

CC = cc
CFLAGS = -Wall -Wextra -Werror

.SECONDARY: ${M_OBJS} ${B_OBJS}

all: ${NAME}

${NAME}: ${M_OBJS}
	ar -rcs ${NAME} ${M_OBJS}

bonus: .bonus


.bonus: ${M_OBJS} ${B_OBJS}
	ar -rcs ${NAME} ${M_OBJS} ${B_OBJS}
	@touch .bonus


%.o: %.c libft.h
	${CC} ${CFLAGS} -c $< -o $@


clean:
	rm -f ${M_OBJS} 


fclean: clean
	rm -f ${NAME} ${B_OBJS} .bonus


re: fclean all


.PHONY: all clean fclean re bonus
